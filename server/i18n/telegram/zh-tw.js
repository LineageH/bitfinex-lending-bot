module.exports = {
  cmdSummary: "顯示放貸摘要",
  cmdEarnings: "收益明細 - 7 天",
  cmdProvided: "已放貸明細",
  cmdSyncEarnings: "同步放貸收益",
  cmdSetReduceRate: "設定自動降利率",
  cmdSubmitOffers: "送出放貸掛單",
  cmdListOffer: "列出未成交掛單",
  cmdHistorys: "列出近期已完結借款",
  summaryTitle: "帳戶摘要 {{symbol}}:",
  balance: "錢包總額",
  available: "可用餘額",
  provided: "已放貸",
  providedRate: "平均利率",
  offered: "掛單中",
  effective: "有效利率",
  marketLow: "市場底價",
  earning30d: "近 30 天收益",
  lifetime: "累計收益",
  fromDate: "由{{date}}起",
  last30Days: "近 30 天",
  summaryError: "取得摘要時發生錯誤。",
  earningsTitle: "收益:",
  providedTitle: "{{symbol}} 已放貸:",
  total: "總計",
  avgRate: "平均利率",
  remaining: "剩餘",
  syncEarningsDone: "放貸收益同步完成",
  submittingOffers: "正在送出放貸掛單",
  submitOffersDone: "放貸掛單送出完成",
  lendingDisabled: "目前設定已停用放貸",
  noOpenOffers: "沒有未成交掛單",
  offersTitle: "掛單列表 ({{symbol}})",
  count: "筆數",
  andMore: "...還有 {{count}} 筆",
  listOfferError: "取得未成交掛單時發生錯誤。",
  historysTitle: "近期已完結借款 ({{symbol}})",
  historysUsage: "用法: /historys [USD|USDT]",
  noFundingHistory: "沒有近期已完結借款紀錄",
  historysError: "取得借款紀錄時發生錯誤。",
  newTransactionsTitle: "新成交 {{symbol}} ({{count}})",
  closedTransactionsTitle: "已完結 {{symbol}} 借款 ({{count}})",
  loanDuration: "歷時",
  tableAmount: "金額",
  tablePeriod: "天期",
  tableRate: "利率",
  tableCount: "筆數",
  tableExpires: "到期",
  frr: "FRR",
  lendingStatus: "放貸狀態",
  autoReduceRateLine: "自動降利率: {{rate}}%",
  setReduceUsage: "用法: /setreducerate <0~-100%> [USD|USDT]",
  setReduceInvalidPercent: "百分比格式錯誤，請輸入 0 到 -100 之間。",
  setReduceInvalidCurrency: "幣別錯誤，請使用 USD 或 USDT。",
  setReduceAutoDisabled: "尚未啟用自動降利率。",
  setReduceUpdated: "已更新自動降利率，並重置下一次倒數。",
  setReduceUpdatedLine: "{{symbol}}: {{rate}}%",
  configReloadTitle: "設定已更新",
  configReloadFileLine: "custom-config.js 已重新載入。",
  configReloadSectionLine: "- {{section}}",
  configReloadValueLine: "  • {{path}}: {{old}} -> {{new}}",
  configReloadAndMorePaths: "  ...另有 {{count}} 項變更",
};
